...
class NodeServer {
  ...
  async start() {
    ...
    Logger.info('Started executing server');
    this.execute();
  }

  async stop() {
    Logger.info('Stopping executing server');
    this.stopped = true;
    if (this.socket) {
      await this.socket.unbind(this.socket.lastEndpoint);
      this.socket.close();
    }
  }
  ...
  async execute() {
    ...
    for await (const [msg] of this.socket) {
      ...
      switch (command) {
        case NODE_SERVER_CMD.REGISTER:
          await this.processRegisterNodeRequest(request);
          break;
        case NODE_SERVER_CMD.READY_TO_WORK:
          await this.processReadyToWorkRequest(request);
          break;
        case NODE_SERVER_CMD.GENERATE_NEXT_MESSAGE_IDX:
  ...
  async processRegisterNodeRequest(request) {
    const { id } = request;
    Logger.debug(`NodeServer: Registration of ${id}`);
  ...
  async initializeNodes() {
    Object.entries(this.config.nodes).forEach(([id, node]) => {
      Logger.debug(`${id}: ${JSON.stringify(node)}`);
...
module.exports = { NodeServer, NODE_SERVER_CMD };
