...
class NodeBase {
  ...
  async generateEvent(event, value, msgId) {
    ...
    Logger.debug('Generating event with value');
    Logger.debug(value);
  ...
  async initializeListener() {
    ...
    for (const item of this.config.input_nodes) {
      ...
      item.edges.forEach((edge) => {
        ...
        Logger.debug(`${this.id} subscribed to ${topic}`);
        this.subSocket.subscribe(topic);
        this.event2handler[topic] = this.handlers[edge.handler];
      });
    }
    // this.processEvents();
  }
  ...
  async processEvents() {
    ...
    Logger.debug(`${this.id} is waiting for the next event`);
